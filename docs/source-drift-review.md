...
Known secret assignments, credential-bearing HTTP headers, cookie headers,
well-known token formats, private-key blocks, and private-transcript markers
are redacted before the diff is stored. Source text is never written to logs. A
...
and audit history. Accept and reopen lock the watched resource and fail if the
proposal's new record is no longer its last successful version. Generic artifact
PATCH requests cannot bypass these Review Inbox lifecycle gates.

All list, lookup, creation, and action queries use the authenticated tenant.
Composite tenant foreign keys enforce the resource and both source-record
pointers at the database layer. The unique `(tenant_id, dedupe_key)` constraint
is the final retry and concurrency guard; a savepoint converts competing inserts
into a successful lookup of the unique winner without rolling back the refresh.
