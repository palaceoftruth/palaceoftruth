...
def upgrade() -> None:
    ...
    # Every delegated record must reference a client from its own tenant.
    op.create_unique_constraint("uq_mcp_clients_id_tenant", "mcp_clients", ["id", "tenant_id"])
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["client_id", "tenant_id"],
            ["mcp_clients.id", "mcp_clients.tenant_id"],
            name="fk_mcp_oauth_interactions_client_tenant",
            ondelete="CASCADE",
        ),
    )
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.TIMESTAMP(timezone=True), nullable=False, server_default=sa.text("now()")),
        sa.UniqueConstraint("id", "tenant_id", name="uq_mcp_oauth_grants_id_tenant"),
        sa.ForeignKeyConstraint(
            ["client_id", "tenant_id"],
            ["mcp_clients.id", "mcp_clients.tenant_id"],
            name="fk_mcp_oauth_grants_client_tenant",
            ondelete="CASCADE",
        ),
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["grant_id", "tenant_id"],
            ["mcp_oauth_delegated_grants.id", "mcp_oauth_delegated_grants.tenant_id"],
            name="fk_mcp_oauth_codes_grant_tenant",
            ondelete="CASCADE",
        ),
    )
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["grant_id", "tenant_id"],
            ["mcp_oauth_delegated_grants.id", "mcp_oauth_delegated_grants.tenant_id"],
            name="fk_mcp_oauth_refresh_families_grant_tenant",
            ondelete="CASCADE",
        ),
    )
    op.add_column("mcp_oauth_access_tokens", sa.Column("delegated_grant_id", postgresql.UUID(as_uuid=True), nullable=True))
    op.create_foreign_key(
        "fk_mcp_oauth_access_tokens_delegated_grant_tenant",
        "mcp_oauth_access_tokens",
        "mcp_oauth_delegated_grants",
        ["delegated_grant_id", "tenant_id"],
        ["id", "tenant_id"],
        ondelete="RESTRICT",
    )
...
def downgrade() -> None:
    op.drop_constraint("fk_mcp_oauth_access_tokens_delegated_grant_tenant", "mcp_oauth_access_tokens", type_="foreignkey")
    ...
    op.drop_constraint("uq_mcp_clients_id_tenant", "mcp_clients", type_="unique")
