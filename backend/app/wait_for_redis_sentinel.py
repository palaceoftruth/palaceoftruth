"""Wait for a writable Sentinel-discovered Redis primary.

This lives under `app/` rather than `scripts/` because both the ARQ worker gate
(scripts/wait_for_worker_dependencies.py) and the API lifespan need it. The
scripts package already imports from `app`, so keeping the shared logic here
preserves that one-way dependency.
"""
