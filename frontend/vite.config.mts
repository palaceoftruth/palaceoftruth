...
// Mirror the deployed nginx proxy in local Vite dev: choose a reachable API target
// and inject the server-side API key so the browser does not need the shared secret.
...
export default defineConfig(async ({ command, mode }) => {
  ...
  if (command === "serve" && !apiKey) {
    console.warn("[vite] API_KEY is not set, proxied API requests will fail auth.");
  }
